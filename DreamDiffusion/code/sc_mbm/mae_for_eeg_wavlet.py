...
class MAEforEEG(nn.Module):
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        mask = mask.repeat(1, imgs.shape[1], 1)
        mse_loss = ((imgs - pred) ** 2 * mask)[:, :15, :] 
        mse_loss = mse_loss.mean()

        zeros = torch.zeros(imgs.shape[0], imgs.shape[1], 1).to("cuda")
        imgs_minuend = torch.cat((zeros, imgs), dim=-1)
        imgs_subtrahend = torch.cat((imgs, zeros), dim=-1)
        imgs_diff = (imgs_minuend - imgs_subtrahend)[:, :, 1:-1] 
        pred_minuend = torch.cat((zeros, pred), dim=-1)
        pred_subtrahend = torch.cat((pred, zeros), dim=-1)
        pred_diff = (pred_minuend - pred_subtrahend)[:, :, 1:-1] 
        mask_diff = mask[:, :, 1:]
        mse_loss_diff = ((imgs_diff - pred_diff) ** 2 * mask_diff)[:, :15, :]
        mse_loss_diff = mse_loss_diff.mean()

        # print(imgs[0][0], pred[0][0])

        return mse_loss + mse_loss_diff
