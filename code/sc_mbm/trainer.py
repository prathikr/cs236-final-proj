...
def valid_one_epoch(model, data_loader, optimizer, device, epoch, 
                        loss_scaler, log_writer=None, config=None):
    total_loss = []
    accum_iter = config.accum_iter
    for data_iter_step, (data_dcit) in enumerate(data_loader):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            loss, _, _ = model(samples, img_features=None, valid_idx=None, mask_ratio=config.mask_ratio)
        ...
        total_loss.append(loss_value)
        if device == torch.device('cuda:0'):
            print('val_loss_step:', np.mean(total_loss))

    if log_writer is not None:
        log_writer.log('val_loss_step', np.mean(total_loss), step=epoch)
    if config.local_rank == 0:        
        print(f'[Epoch {epoch}] val loss: {np.mean(total_loss)}')