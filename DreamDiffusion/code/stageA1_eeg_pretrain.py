...
from sc_mbm.trainer import train_one_epoch, valid_one_epoch
...
def main(config):
    ...
    dataset_pretrain = eeg_pretrain_dataset(path='datasets/moabb_data_train/', roi=config.roi, patch_size=config.patch_size,
    ...
    dataset_pretrain_val = eeg_pretrain_dataset(path='datasets/moabb_data_val/', roi=config.roi, patch_size=config.patch_size,
                transform=fmri_transform, aug_times=config.aug_times, num_sub_limit=config.num_sub_limit, 
                include_kam=config.include_kam, include_hcp=config.include_hcp)
    ...
    dataloader_eeg_val = DataLoader(dataset_pretrain_val, batch_size=config.batch_size, shuffle=False, pin_memory=True)
    ...
    for ep in range(config.num_epoch):
        ...
        cor_list.append(cor)
        valid_one_epoch(model, dataloader_eeg_val, optimizer, device, ep, loss_scaler, logger, config)
        if (ep % 5 == 0 or ep + 1 == config.num_epoch) and config.local_rank == 0: #and ep != 0
            # save models
        # if True:
            # print("SAVING MODEL TO", os.path.join(output_path,'checkpoints'))
...
def plot_recon_figures(model, device, dataset, output_path, num_figures = 5, config=None, logger=None, model_without_ddp=None):
    ...
    # run = Run.get_context()
    # run.upload_file(name=f'{fig_name}.png', path_or_stream=os.path.join(output_path, f'{fig_name}.png'))
    if logger is not None:
